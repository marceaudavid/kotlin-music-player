...
class SongAdapter(
    ...
    override fun onBindViewHolder(holder: SongViewHolder, position: Int) {
        ...
        holder.title.text =
            songList[position].description.extras?.getString(MediaMetadata.METADATA_KEY_TITLE)
        holder.artist.text =
            songList[position].description.extras?.getString(MediaMetadata.METADATA_KEY_ARTIST)
        Glide.with(mainActivity)
            .load(songList[position].description.extras?.getString(MediaMetadata.METADATA_KEY_ART_URI))
            .centerCrop().into(holder.art)
